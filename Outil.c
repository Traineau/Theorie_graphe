...
char *sChaine0(char *sTexte,int nLargeurHorsTout){
	//rend nLargeurHorsTout caractères commençant par sTexte et finissant par des blancs. Tronque sTexte si trop grand.
...
char *sChaine1(char *sTexte,int nLargeurHorsTout){
	//rend nLargeurHorsTout caractères commençant par des blancs et finissant par sTexte. Tronque sTexte si trop grand.
	Assert1("sChaine1 appel",0<=nLargeurHorsTout);
	int nC,nCarLim,nTexteLg;
	char *sChaine1;
...
char *sChaineEtale(char *sChaine){
	return(sChaineEtaler(sChaine,1));
}//sChaineEtale

char *sChaineEtale2(char *sChaine){
	return(sChaineEtaler(sChaine,2));
}//sChaineEtale2

char *sChaineEtaler(char *sChaine,int nEspaceEntreCaractere){
	//rend sChaine[nDebutInclus,nFinIncluse],ou une chaîne vide si non défini 
	int nC,nD,uE;
	int nCarat;
	char *sEtale;
	int nRequis;
	//tt("sChaineEtaler",sG(sChaine));
	Assert2("sChaineEtaler0",nEspaceEntreCaractere>=0,nCaractere(sChaine)==nChaineLg(sChaine));//chaque caractère est codé sur 1 code caractère (pas de "é")
	nCarat=nChaineLg(sChaine);
	nRequis=(nCarat>0)? nCarat+nEspaceEntreCaractere*(nCarat-1): nCarat;
	//tee("nCarat,nRequis",nCarat,nRequis);
	sEtale=malloc(nRequis+kuAntiSlashZerO);
		...
		Assert1("sChaineEtaler1",nD==nRequis);
	MallocGereR(sEtale);
	return(sEtale);
}//sChaineEtaler
...
char *sEnt3(long yEntier,int nCaractere){
	...
	Assert1("sEnt3",0);
	char *sEnt3=sChaine1(sPar3(yEntier),nCaractere);
	MallocGereR(sEnt3);
	return(sEnt3);
}//sEnt3
...
char *sReel(float fReel){
	//convertit fReel en chaîne de caractères: sQ(1/2) rend "1/2" et sQ(2/1) rend "2".
	#define kuRequis 31
...
void Titrer2(char *sTitre,char cMotif){
	...
	const int kuTitreLargeurPaire=80;//ou un autre nombre pair
...
char *sEuro2(float fReel, int nCaractereEnTout){
	...
	int mPos;
	char *sEuro2;
	sprintf(sEuro2, "%.2f", fReel);
	sEuro2 = sChaine2(sEuro2, nCaractereEnTout);
	mPos = mChainePos('.', sEuro2);
	Assert1("sEuro2", mPos > 0);
	sEuro2 = sC3(sChainon(sEuro2, 0, mPos-1), "€", sChainon(sEuro2, mPos+1, nChaineLg(sEuro2) - 1));
	MallocGereR(sEuro2);
	return (sEuro2);
}
